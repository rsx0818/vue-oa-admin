...
export default {
  ...
  methods: {
    ...
    fetchData(page = 1) {
      ...
      api.fetchData(this.page, this.limit, this.searchObj).then(
        response => {
    ...
    getRoles() {
      roleApi.getRoles(this.sysUser.id).then(response => {
        const { allRoleList, assignRoleList } = response.data
        console.log(allRoleList)
        console.log(assignRoleList)
        this.allRoles = allRoleList
        this.userRoleIds = assignRoleList.map(item => item.id)
        this.checkAll = allRoleList.length === assignRoleList.length
        this.isIndeterminate = assignRoleList.length > 0 && assignRoleList.length < allRoleList.length
      })
    },
